...
public class EmotionDiaryController {
     ...
     * 获取指定月份的情绪日历数据
     */
    @Operation(summary = "获取指定月份的情绪日历数据", description = "获取当前用户指定月份每天的情绪状态")
    @GetMapping("/calendar")
    public Result<Map<String, String>> getEmotionCalendar(
            @Parameter(description = "年份") @RequestParam("year") int year,
            @Parameter(description = "月份") @RequestParam("month") int month) {
        log.info("收到获取情绪日历请求，年份: {}, 月份: {}", year, month);
        ...
        try {
            ...
            queryDTO.setSize(31L); // 单月最多31天，一页足够查询完

            Page<EmotionDiaryResponseDTO> page = emotionDiaryService.selectPage(queryDTO);

            Map<String, String> resultMap = new HashMap<>();
            if (page != null && page.getRecords() != null) {
                for (EmotionDiaryResponseDTO dto : page.getRecords()) {
                    if (dto.getDiaryDate() != null && dto.getDominantEmotion() != null && !dto.getDominantEmotion().isEmpty()) {
                        resultMap.put(dto.getDiaryDate().toString(), dto.getDominantEmotion());
                    }
                }
            }
            return Result.success(resultMap);
        } catch (Exception e) {
            log.error("获取情绪日历数据失败: {}", e.getMessage(), e);
            return Result.error("获取情绪日历失败: " + e.getMessage());
        }
    }
